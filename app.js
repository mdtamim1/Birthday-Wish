...
function updateGiftSurprisePhoto() {
  const photoEl = document.getElementById('giftBoxRevealedImg');
  const captionEl = document.getElementById('revealedGiftCaption');
  const msgEl = document.getElementById('giftMessage');
  const fallbackSrc = S.photo || (S.photos && S.photos[0]) || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=600&q=80';
  
  const targetPhoto = S.giftPhoto || fallbackSrc;
  if (photoEl) {
    photoEl.src = targetPhoto;
  }
  if (captionEl) {
    captionEl.textContent = S.name ? `Special Surprise for ${S.name} 💕` : 'Special Surprise 💕';
  }
  if (msgEl) {
    msgEl.textContent = S.giftMessage || DEFAULT_SETTINGS.giftMessage;
  }
}
window.updateGiftSurprisePhoto = updateGiftSurprisePhoto;

function initGiftUnbox() {
  const giftBoxVisual = document.getElementById('giftBoxVisual');
  const giftOpenBtn = document.getElementById('giftOpenBtn');
  const giftLidWrap = document.getElementById('giftLidWrap');
  const revealedGiftCard = document.getElementById('revealedGiftCard');
  const giftHeading = document.getElementById('giftHeading');
  const giftBoxBody = document.getElementById('giftBoxBody');
  ...
  function triggerUnbox(e) {
    ...
    if (isGiftUnboxed) {
      // If already opened, clicking opens the full lightbox
      const photoEl = document.getElementById('giftBoxRevealedImg');
      if (photoEl && photoEl.src) {
        openLightbox(photoEl.src, S.name ? `Surprise for ${S.name} 🎁` : 'Surprise Gift ✨');
      }
      return;
    }
    ...
    // 1. Shake gift box vigorously
    if (typeof gsap !== 'undefined') {
      gsap.to(giftBoxVisual, {
        rotation: 10,
        duration: 0.07,
        yoyo: true,
        repeat: 7,
        ease: 'power1.inOut',
        onComplete: () => {
          gsap.set(giftBoxVisual, { rotation: 0 });
          performUnbox();
        }
      });
    } else {
  ...
  function performUnbox() {
    // 2. Fly 3D lid off
    if (giftLidWrap && typeof gsap !== 'undefined') {
      gsap.to(giftLidWrap, {
        y: -120,
        x: 50,
        rotationZ: 42,
      ...
      giftLidWrap.style.transform = 'translateY(-110px) rotate(40deg)';
      giftLidWrap.style.opacity = '0';
    }

    // 3. Reveal floating surprise photo card (rising from inside box)
    if (revealedGiftCard) {
      revealedGiftCard.style.display = 'block';
      if (typeof gsap !== 'undefined') {
        gsap.fromTo(revealedGiftCard,
          { y: 40, scale: 0.15, opacity: 0 },
          { 
            y: -95, 
            scale: 1.15, 
            opacity: 1, 
            ...
            ease: 'back.out(2.4)',
            onComplete: () => {
              gsap.to(revealedGiftCard, { y: -85, duration: 2.2, repeat: -1, yoyo: true, ease: 'sine.inOut' });
            }
          }
        );
      }
    ...
    // 4. Confetti shower & sound
    launchConfetti(220);
    sfx.playWhoosh();

    // 5. Update text & button
    if (giftHeading) {
      giftHeading.textContent = '🎉 Unwrapped with Love! 💕';
      giftHeading.style.color = '#fef08a';
    }
    if (giftOpenBtn) {
      giftOpenBtn.innerHTML = `
        <span class="btn-glow-layer"></span>
        <span class="btn-icon">💖</span>
        <span class="btn-text">View Full Surprise Photo</span>
        <span class="btn-sparkle">🔍</span>
      `;
      giftOpenBtn.style.background = 'linear-gradient(135deg, #10b981, #34d399)';
    }

    // Click on revealed photo card directly opens full image lightbox
    revealedGiftCard?.addEventListener('click', (e) => {
      e.stopPropagation();
      const photoEl = document.getElementById('giftBoxRevealedImg');
      if (photoEl && photoEl.src) {
        openLightbox(photoEl.src, S.name ? `Surprise for ${S.name} 🎁` : 'Surprise Gift ✨');
  ...
  if (giftBoxVisual) {
    giftBoxVisual.addEventListener('click', triggerUnbox);
  }
  if (giftOpenBtn) {
    giftOpenBtn.addEventListener('click', triggerUnbox);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initGiftUnbox);
} else {
  initGiftUnbox();
}
